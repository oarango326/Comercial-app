...
<h3>Cobro:# {{$cobro->id}}</h3>
        ...
        <div class="row">
            <div class="col-xs-12 col-md-4 col-lg-4 form">
                ...
                <input type="text" name="cliente" id="cliente" class="form-control" value="{{$cliente[0]->nombre}}" readonly>
                {{-- <button class="btn btn-primary" id="btnSeleccion" role="button">Seleccionar</button> --}}
            </div>
            <div class="col-xs-12  col-sm-12 col-md-8 col-lg-6">
                <label for="lblcliente">Direccion</label>
                <input type="text" name="direccion" id="direccion" class="form-control" value="{{$cliente[0]->direccion}} {{$cliente[0]->ciudad}} {{$cliente[0]->estado}} {{$cliente[0]->telefono}}" readonly>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-12 col-lg-2">
                <label for="lblcliente">Fecha Cobro</label>
                <input type="date" name="fechacobro" id="fechacobro" class="form-control" value="{{$cobro->fechacobro}}" readonly>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-12 col-md-2 col-lg-2">
                <label for="lbltipodocumento">Tipo Documento</label>
                <input type="text" name="tipodocumento" id="tipodocumento" class="form-control" value="{{($cobro->tipodocumento=='fac')?'FACTURA':'ALBARAN'}}" readonly>
            </div>
            <div class="col-xs-12 col-md-4 col-lg-4 form">
                <label for="lblnumdocumento">#Nota</label>
            <input type="text" name="numdocumento" id="numdocumento" class="form-control" maxlength="10" value="{{$cobro->numdocumento}}" readonly>
            </div>
            <div class="col-xs-12 col-sm-12 col-md-2 col-lg-2">
                <label for="lblcliente">Fecha Documento</label>
                <input type="date" name="fechadocumento" id="fechadocumento" class="form-control" value="{{$cobro->fechadocumento}}" readonly>
            </div>
            <div class="col-xs-12 col-md-4 col-lg-4 ">
                <label for="lbltipocobro">Tipo Cobro</label>
                <input type="text" name="tipocobro" id="tipocobro" class="form-control" value="{{($cobro->tipocobro=='efectivo')?'EFECTIVO':'PAGARE'}}" readonly>
            </div>
        </div>
        <div class="row">
            <div class="col-xs-12 col-md-4 col-lg-4 ">
                <label for="lblmonto">Monto Nota</label>
                <input type="text" name="monto" id="monto" class="form-control montos calculo" value="{{$cobro->monto}}" readonly>
            </div>
            <div class="col-xs-12 col-md-4 col-lg-4 ">
                <label for="lblmontoabono">Monto Abono</label>
                <input type="text" name="abono" id="abono" class="form-control montos calculo" value="{{$cobro->abono}}" readonly>
            </div>
            <div class="col-xs-12 col-md-4 col-lg-4 ">
                <label for="lbltotal">Total cobrado</label>
                <input type="text" name="total" id="total"
                class="form-control montos calculo" value="{{$cobro->total}}" readonly>
            </div>
        </div>
        <div class="row">
